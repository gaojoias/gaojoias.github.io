...
const MKT_PLATFORMS = {
  ...
  threads:   { label: 'Threads',   icon: 'fa-brands fa-threads',   color: '#101010' },
  whatsapp:  { label: 'WhatsApp',  icon: 'fa-brands fa-whatsapp',  color: '#25d366' },
};
const MKT_STATUS_LABELS = { draft: 'Rascunho', scheduled: 'Agendado', published: 'Publicado', cancelled: 'Cancelado' };
const MKT_STATUS_COLORS = { draft: 'muted', scheduled: 'blue', published: 'green', cancelled: 'red' };
const MKT_GOAL_LABELS   = { awareness: 'Alcance', engagement: 'Engajamento', conversion: 'Conversão', followers: 'Seguidores', traffic: 'Tráfego' };
...
function mktKpi() {
  const published = state.marketing.posts.filter(p => p.status === 'published');
  ...
  const totalRev    = metrics.reduce((s, m) => s + m.revenueAttributedCents, 0);
  const engRate     = totalImpr > 0 ? ((totalLikes + metrics.reduce((s,m)=>s+m.commentsCount+m.saves,0)) / totalImpr * 100) : 0;
  return { published: published.length, totalReach, totalImpr, engRate, totalRev };
}
...
function renderMktOverview() {
  const kpi = mktKpi();
  const kpiEl = document.getElementById('mkt-kpi-row');
  if (!kpiEl) return;
  kpiEl.innerHTML = `
    <div class="mkt-kpi">
      <div class="mkt-kpi-icon" style="background:rgba(99,102,241,.15);color:#6366f1"><i class="fa-solid fa-images"></i></div>
      <div><span class="mkt-kpi-val">${kpi.published}</span><span class="mkt-kpi-label">Posts publicados</span></div>
    </div>
    <div class="mkt-kpi">
      <div class="mkt-kpi-icon" style="background:rgba(16,185,129,.15);color:#10b981"><i class="fa-solid fa-eye"></i></div>
      <div><span class="mkt-kpi-val">${fmtNum(kpi.totalReach)}</span><span class="mkt-kpi-label">Alcance total</span></div>
    </div>
    <div class="mkt-kpi">
      <div class="mkt-kpi-icon" style="background:rgba(245,158,11,.15);color:#f59e0b"><i class="fa-solid fa-heart"></i></div>
      <div><span class="mkt-kpi-val">${kpi.engRate.toFixed(1)}%</span><span class="mkt-kpi-label">Taxa de engajamento</span></div>
    </div>
    <div class="mkt-kpi">
      <div class="mkt-kpi-icon" style="background:rgba(239,68,68,.15);color:#ef4444"><i class="fa-solid fa-sack-dollar"></i></div>
      <div><span class="mkt-kpi-val">${fmtBRL(kpi.totalRev)}</span><span class="mkt-kpi-label">Receita atribuída</span></div>
    </div>
    <div class="mkt-kpi">
      <div class="mkt-kpi-icon" style="background:rgba(99,102,241,.15);color:#6366f1"><i class="fa-solid fa-chart-bar"></i></div>
      <div><span class="mkt-kpi-val">${fmtNum(kpi.totalImpr)}</span><span class="mkt-kpi-label">Impressões totais</span></div>
    </div>
  `;

  const recent = [...state.marketing.posts]
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    .slice(0, 6);
  const recentEl = document.getElementById('mkt-recent-posts');
  if (recentEl) {
    recentEl.innerHTML = recent.length ? recent.map(p => mktPostCardHtml(p)).join('') : '<p class="muted" style="padding:12px">Nenhuma postagem ainda.</p>';
  }

  const activeCamps = state.marketing.campaigns.filter(c => c.status === 'active').slice(0, 4);
  const campsEl = document.getElementById('mkt-active-campaigns');
  if (campsEl) {
    campsEl.innerHTML = activeCamps.length ? activeCamps.map(c => mktCampaignMiniHtml(c)).join('') : '<p class="muted" style="padding:12px">Nenhuma campanha ativa.</p>';
  }
}
...
function mktPostCardHtml(p) {
  const plat = MKT_PLATFORMS[p.platform] || MKT_PLATFORMS.instagram;
  const statusColor = MKT_STATUS_COLORS[p.status] || 'muted';
  const thumb = p.imageUrls?.[0] ? `<img src="${escapeHtml(p.imageUrls[0])}" class="mkt-card-thumb" alt="">` : `<div class="mkt-card-thumb mkt-thumb-placeholder"><i class="${plat.icon}"></i></div>`;
  const m = mktPostMetrics(p.id);
  return `<div class="mkt-post-card" data-post-id="${p.id}">
    ${thumb}
    <div class="mkt-card-body">
      <div class="mkt-card-top">
        <span class="mkt-plat-badge" style="color:${plat.color}"><i class="${plat.icon}"></i> ${plat.label}</span>
        <span class="badge badge-${statusColor}">${MKT_STATUS_LABELS[p.status]}</span>
      </div>
      <p class="mkt-card-title">${escapeHtml(p.title || p.caption?.slice(0,60) || '(sem título)')}</p>
      ${p.scheduledAt ? `<span class="mkt-card-date"><i class="fa-solid fa-clock"></i> ${fmtDatetime(p.scheduledAt)}</span>` : ''}
      ${m ? `<div class="mkt-card-metrics"><span><i class="fa-solid fa-eye"></i>${fmtNum(m.reach)}</span><span><i class="fa-solid fa-heart"></i>${fmtNum(m.likes)}</span><span><i class="fa-solid fa-bookmark"></i>${fmtNum(m.saves)}</span></div>` : ''}
    </div>
    <div class="mkt-card-actions">
      <button class="btn btn-ghost btn-xs" data-mkt-action="edit-post" data-id="${p.id}" title="Editar"><i class="fa-solid fa-pen"></i></button>
      <button class="btn btn-ghost btn-xs" data-mkt-action="metrics-post" data-id="${p.id}" title="Métricas"><i class="fa-solid fa-chart-bar"></i></button>
...
function mktCampaignMiniHtml(c) {
  const goalLabel = MKT_GOAL_LABELS[c.goal] || c.goal;
  const progress = c.budgetCents > 0 ? Math.min(100, (c.revenueCents / c.budgetCents) * 100) : 0;
  return `<div class="mkt-camp-mini">
    <div class="mkt-camp-mini-head">
      <strong>${escapeHtml(c.name)}</strong>
      <span class="mkt-goal-pill">${goalLabel}</span>
    </div>
    <div class="mkt-camp-mini-stats">
      <span>${c.postsCount} posts</span>
      <span>${fmtNum(c.totalReach)} alcance</span>
      <span>${fmtBRL(c.revenueCents)} receita</span>
    </div>
    ${c.budgetCents > 0 ? `
    <div class="mkt-progress-wrap">
      <div class="mkt-progress-bar" style="width:${progress}%"></div>
    </div>
    <div class="mkt-progress-label">ROI: ${fmtBRL(c.revenueCents)} de ${fmtBRL(c.budgetCents)} investidos</div>` : ''}
  </div>`;
}
...
function renderMktCalendar() {
  ...
  label.textContent = `${months[state.mktCalMonth]} ${state.mktCalYear}`;

  const year  = state.mktCalYear;
  const month = state.mktCalMonth;
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    html += `<div class="mkt-cal-cell${isToday ? ' today' : ''}" data-mkt-day="${day}">
      <span class="mkt-cal-day-num">${day}</span>
      <div class="mkt-cal-pills">
        ${posts.slice(0, 3).map(p => {
          const plat = MKT_PLATFORMS[p.platform] || MKT_PLATFORMS.instagram;
          return `<div class="mkt-cal-pill" style="border-left-color:${plat.color}" data-mkt-action="edit-post" data-id="${p.id}" title="${escapeHtml(p.title || p.platform)}">
            <i class="${plat.icon}" style="color:${plat.color}"></i>
            <span>${escapeHtml(p.title?.slice(0,18) || MKT_TYPE_LABELS[p.postType])}</span>
          </div>`;
        }).join('')}
        ${posts.length > 3 ? `<div class="mkt-cal-pill-more">+${posts.length - 3}</div>` : ''}
      </div>
      <button class="mkt-cal-add" data-mkt-action="new-post-day" data-day="${day}" title="Adicionar post neste dia"><i class="fa-solid fa-plus"></i></button>
    </div>`;
  }
  grid.innerHTML = html;
}
...
function renderMktPostagens() {
  const tbody = document.getElementById('mkt-posts-tbody');
  if (!tbody) return;

  const platFilter   = (document.getElementById('mkt-filter-platform')?.value || '');
  const statusFilter = (document.getElementById('mkt-filter-status')?.value || '');
  ...
  tbody.innerHTML = posts.map(p => {
    const plat  = MKT_PLATFORMS[p.platform] || MKT_PLATFORMS.instagram;
    const sc    = MKT_STATUS_COLORS[p.status] || 'muted';
    const m     = mktPostMetrics(p.id);
    const thumb = p.imageUrls?.[0]
      ? `<img src="${escapeHtml(p.imageUrls[0])}" class="mkt-table-thumb" alt="">`
      : `<div class="mkt-table-thumb mkt-thumb-placeholder sm"><i class="${plat.icon}"></i></div>`;
    return `<tr>
      <td>${thumb}</td>
      <td><strong>${escapeHtml(p.title || '(sem título)')}</strong>${p.campaignName ? `<br><small class="muted">${escapeHtml(p.campaignName)}</small>` : ''}</td>
      <td><span style="color:${plat.color}"><i class="${plat.icon}"></i> ${plat.label}</span></td>
      <td>${MKT_TYPE_LABELS[p.postType] || p.postType}</td>
      <td><span class="badge badge-${sc}">${MKT_STATUS_LABELS[p.status]}</span></td>
      <td>${p.scheduledAt ? fmtDatetime(p.scheduledAt) : '<span class="muted">—</span>'}</td>
      <td>${p.campaignName ? escapeHtml(p.campaignName) : '<span class="muted">—</span>'}</td>
      <td>${m ? fmtNum(m.reach) : '<span class="muted">—</span>'}</td>
  ...
  }).join('') || `<tr><td colspan="9" class="empty-row">Nenhuma postagem encontrada.</td></tr>`;
}
...
function renderMktCampanhas() {
  ...
  if (!grid) return;
  const camps = [...state.marketing.campaigns].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  grid.innerHTML = camps.map(c => {
    const statusColor = { draft:'muted', active:'green', paused:'blue', completed:'purple', cancelled:'red' }[c.status] || 'muted';
    const progress = c.budgetCents > 0 ? Math.min(100, (c.revenueCents / c.budgetCents) * 100) : 0;
    const roi = c.budgetCents > 0 ? ((c.revenueCents - c.budgetCents) / c.budgetCents * 100).toFixed(0) : null;
    return `<div class="mkt-camp-card">
      <div class="mkt-camp-card-header">
        ...
        <div class="mkt-camp-header-right">
          <span class="badge badge-${statusColor}">${c.status === 'active' ? 'Ativa' : c.status === 'paused' ? 'Pausada' : c.status === 'completed' ? 'Concluída' : c.status === 'draft' ? 'Rascunho' : 'Cancelada'}</span>
      ...
      <div class="mkt-camp-meta">
        <span><i class="fa-solid fa-bullseye"></i> ${MKT_GOAL_LABELS[c.goal] || c.goal}</span>
        ${c.startDate ? `<span><i class="fa-solid fa-calendar"></i> ${fmtDate(c.startDate)}${c.endDate ? ' → ' + fmtDate(c.endDate) : ''}</span>` : ''}
        ${c.budgetCents ? `<span><i class="fa-solid fa-wallet"></i> Verba: ${fmtBRL(c.budgetCents)}</span>` : ''}
      </div>
      <div class="mkt-camp-stats-row">
        ...
        <div class="mkt-camp-stat"><span class="mkt-camp-stat-val">${fmtNum(c.totalReach)}</span><span class="mkt-camp-stat-label">Alcance</span></div>
        <div class="mkt-camp-stat"><span class="mkt-camp-stat-val">${fmtBRL(c.revenueCents)}</span><span class="mkt-camp-stat-label">Receita</span></div>
        ${roi !== null ? `<div class="mkt-camp-stat"><span class="mkt-camp-stat-val ${roi >= 0 ? 'green' : 'red'}">${roi >= 0 ? '+' : ''}${roi}%</span><span class="mkt-camp-stat-label">ROI</span></div>` : ''}
      </div>
      ${c.budgetCents > 0 ? `
      <div class="mkt-progress-wrap">
        <div class="mkt-progress-bar" style="width:${progress}%"></div>
  ...
  }).join('') || `<p class="muted" style="padding:20px">Nenhuma campanha criada ainda.</p>`;
}

// ── Modals ───────────────────────────────────────────────────

function openPostModal(post = null, prefillDate = null) {
  const camps = state.marketing.campaigns;
  const isEdit = !!post;
  const sched = post?.scheduledAt ? post.scheduledAt.slice(0, 16)
    : prefillDate ? `${state.mktCalYear}-${String(state.mktCalMonth + 1).padStart(2,'0')}-${String(prefillDate).padStart(2,'0')}T12:00`
    : '';
  const imageUrlsVal = (post?.imageUrls || []).join('\n');

  openModal(`
    <h3>${isEdit ? 'Editar postagem' : 'Nova postagem'}</h3>
    <form id="mkt-post-form" class="form-grid">
      <input type="hidden" name="id" value="${post?.id || ''}">
      <label class="full">Título<input type="text" name="title" value="${escapeHtml(post?.title || '')}" placeholder="Ex: Promoção de verão - Conjunto Safira"></label>
      <label>Plataforma
      ...
      <label>Status
        <select name="status">
          ${Object.entries(MKT_STATUS_LABELS).map(([k,v]) => `<option value="${k}" ${(post?.status || 'draft') === k ? 'selected' : ''}>${v}</option>`).join('')}
        </select>
      </label>
      <label>Agendado para<input type="datetime-local" name="scheduledAt" value="${escapeHtml(sched)}"></label>
      <label>Publicado em<input type="datetime-local" name="publishedAt" value="${escapeHtml(post?.publishedAt?.slice(0,16) || '')}"></label>
      <label>Campanha
      ...
      <label>Tags<input type="text" name="tags" value="${escapeHtml(post?.tags || '')}" placeholder="Ex: joias, promo, verao"></label>
      <label class="full">Legenda / Caption<textarea name="caption" rows="4" placeholder="Texto da postagem...">${escapeHtml(post?.caption || '')}</textarea></label>
      <label class="full">URLs das imagens <small class="muted">(uma por linha — use o upload de fotos do admin)</small>
        <textarea name="imageUrls" rows="3" placeholder="img/uploads/exemplo.jpg">${escapeHtml(imageUrlsVal)}</textarea>
      </label>
      <label class="full">Observações internas<textarea name="notes" rows="2">${escapeHtml(post?.notes || '')}</textarea></label>
      <div class="form-actions full">
        <button type="button" class="btn btn-ghost" id="mkt-modal-cancel">Cancelar</button>
        <button type="submit" class="btn btn-primary">${isEdit ? 'Salvar alterações' : 'Criar postagem'}</button>
      </div>
    </form>
  `);

  document.getElementById('mkt-modal-cancel')?.addEventListener('click', closeModal);
  document.getElementById('mkt-post-form')?.addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      title: fd.get('title'), caption: fd.get('caption'), platform: fd.get('platform'),
      postType: fd.get('postType'), status: fd.get('status'),
      scheduledAt: fd.get('scheduledAt'), publishedAt: fd.get('publishedAt'),
      campaignId: fd.get('campaignId'), tags: fd.get('tags'), notes: fd.get('notes'),
      imageUrls,
    };
    try {
      const saved = await apiRequest('upsertPost', payload);
      const idx = state.marketing.posts.findIndex(p => p.id === saved.id);
      if (idx >= 0) state.marketing.posts[idx] = saved; else state.marketing.posts.unshift(saved);
      closeModal();
      renderMarketing();
...
function openCampaignModal(campaign = null) {
  ...
  openModal(`
    <h3>${isEdit ? 'Editar campanha' : 'Nova campanha'}</h3>
    <form id="mkt-camp-form" class="form-grid">
      <input type="hidden" name="id" value="${campaign?.id || ''}">
      <label class="full">Nome da campanha<input type="text" name="name" value="${escapeHtml(campaign?.name || '')}" required placeholder="Ex: Coleção Verão 2026"></label>
      <label class="full">Descrição<textarea name="description" rows="2">${escapeHtml(campaign?.description || '')}</textarea></label>
      <label>Objetivo
        <select name="goal">
          ${Object.entries(MKT_GOAL_LABELS).map(([k,v]) => `<option value="${k}" ${(campaign?.goal || 'engagement') === k ? 'selected' : ''}>${v}</option>`).join('')}
        </select>
      </label>
      <label>Status
        <select name="status">
      ...
      <label>Início<input type="date" name="startDate" value="${campaign?.startDate || ''}"></label>
      <label>Fim<input type="date" name="endDate" value="${campaign?.endDate || ''}"></label>
      <label>Verba (R$)<input type="number" step="0.01" min="0" name="budget" value="${campaign?.budgetCents ? (campaign.budgetCents/100).toFixed(2) : ''}"></label>
      <div class="form-actions full">
  ...
  document.getElementById('mkt-camp-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (idx >= 0) state.marketing.campaigns[idx] = saved; else state.marketing.campaigns.unshift(saved);
      closeModal();
      renderMarketing();
...
function openMetricsModal(postId) {
  ...
  const metrics = mktPostMetrics(postId);
  ...
  openModal(`
    ...
    <p class="muted" style="margin-bottom:16px">Insira os dados copiados do Instagram Insights ou da plataforma correspondente.</p>
    <form id="mkt-metrics-form" class="form-grid">
      <input type="hidden" name="postId" value="${postId}">
      <input type="hidden" name="id" value="${metrics?.id || ''}">
      <label>Alcance<input type="number" min="0" name="reach" value="${metrics?.reach || 0}"></label>
      <label>Impressões<input type="number" min="0" name="impressions" value="${metrics?.impressions || 0}"></label>
      <label>Curtidas<input type="number" min="0" name="likes" value="${metrics?.likes || 0}"></label>
      <label>Comentários<input type="number" min="0" name="commentsCount" value="${metrics?.commentsCount || 0}"></label>
      <label>Salvamentos<input type="number" min="0" name="saves" value="${metrics?.saves || 0}"></label>
      <label>Compartilhamentos<input type="number" min="0" name="shares" value="${metrics?.shares || 0}"></label>
      <label>Cliques no link<input type="number" min="0" name="clicks" value="${metrics?.clicks || 0}"></label>
      <label>Visitas ao perfil<input type="number" min="0" name="profileVisits" value="${metrics?.profileVisits || 0}"></label>
      <label>Novos seguidores<input type="number" min="0" name="newFollowers" value="${metrics?.newFollowers || 0}"></label>
      <label>Receita atribuída (R$)<input type="number" min="0" step="0.01" name="revenueAttributed" value="${metrics?.revenueAttributedCents ? (metrics.revenueAttributedCents/100).toFixed(2) : '0'}"></label>
      <label class="full">Observações<textarea name="notes" rows="2">${escapeHtml(metrics?.notes || '')}</textarea></label>
      <div class="form-actions full">
        <button type="button" class="btn btn-ghost" id="mkt-met-cancel">Cancelar</button>
        <button type="submit" class="btn btn-primary">Salvar métricas</button>
      </div>
    </form>
  `);
  ...
  document.getElementById('mkt-metrics-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const idx = state.marketing.metrics.findIndex(m => m.postId === postId);
      if (idx >= 0) state.marketing.metrics[idx] = saved; else state.marketing.metrics.push(saved);
      closeModal();
      renderMarketing();
      showToast('Métricas salvas.');
    } catch (err) { showToast(err.message || 'Erro ao salvar métricas.', 'error'); }
  });
}

// ── Event delegation for marketing section ────────────────────
...
function initMktEvents() {
  ...
  section.addEventListener('click', (e) => {
    ...
    if (action === 'edit-post') {
    ...
    } else if (action === 'metrics-post') {
      openMetricsModal(id);
    } else if (action === 'delete-post') {
      const post = state.marketing.posts.find(p => p.id === id);
      if (!confirm(`Excluir postagem "${post?.title || '#'+id}"? Esta ação não pode ser desfeita.`)) return;
      apiRequest('deletePost', { id })
        .then(() => { state.marketing.posts = state.marketing.posts.filter(p => p.id !== id); renderMarketing(); showToast('Postagem excluída.'); })
      ...
      apiRequest('deleteCampaign', { id })
        .then(() => { state.marketing.campaigns = state.marketing.campaigns.filter(c => c.id !== id); renderMarketing(); showToast('Campanha excluída.'); })
        .catch(err => showToast(err.message, 'error'));
    } else if (action === 'new-post-day') {
      openPostModal(null, Number(actionEl.dataset.day));
    }
  });

  // New post buttons
  section.querySelectorAll('.mkt-new-post-btn').forEach(btn => {
    btn.addEventListener('click', () => openPostModal());
  });
  section.querySelectorAll('.mkt-new-campaign-btn').forEach(btn => {
    btn.addEventListener('click', () => openCampaignModal());
  });
  ...
  // Filter dropdowns in postagens tab
  ['mkt-filter-platform', 'mkt-filter-status'].forEach(id => {
    document.getElementById(id)?.addEventListener('change', renderMktPostagens);
  });
}
